	...
	_createClass(GenericDatasource, [{
		...
		value: function query(options) {
			...
			(0, _lodash2.default)(options.targets).forEach(function (target) {
				...
				var dimension = {};
				_lodash2.default.each(target.dimensions, function (dim) {
				...
				_lodash2.default.each(_lodash2.default.keys(query), function (key) {
			...
			var allQueryPromise = _lodash2.default.map(queries, function (query) {
				var options = {
					method: 'GET',
					url: query,
					headers: {
			...
			return this.q.all(allQueryPromise).then(function (allResponse) {
				...
				_lodash2.default.each(allResponse, function (response, index) {
					...
					var resResult = [];
					_lodash2.default.each(selectedFieldNames, function (selectedFieldName) {
						var datapoints = [];
						var Datapoints = JSON.parse(response.data.Datapoints);
						_lodash2.default.each(Datapoints, function (Datapoint) {
							var datapoint = [];
		...
		value: function ListProjectDetail() {
			...
			return this.backendSrv.datasourceRequest({
				...
				var result = [];
				_lodash2.default.each(data.Resources.Resource, function (Resource) {
		...
		value: function ListMetricDetail(parameters) {
			...
			_lodash2.default.each(_lodash2.default.keys(parameters), function (key) {
		...
		value: function metricFindQuery(query) {
			...
			var dimensionValues = [];
			_lodash2.default.each(items[1].split(","), function (i) {
			...
			var arr = pm.split("/");
			var dimensionKeys = _lodash2.default.filter(["userId", "instanceId"], function (key) {
				return key !== 'userId';
			});
			var dimension = {};
			var j = 1;
			...
			_lodash2.default.each(_lodash2.default.keys(param), function (key) {
			...
			return this.backendSrv.datasourceRequest({
				...
				if (data.Code == 200) {
					var dimList = [];
					_lodash2.default.each(response.data.Datapoints, function (Datapoint) {
		...
		value: function buildRealUrl(param) {
			var signer = new _signer.CmsSigner({
